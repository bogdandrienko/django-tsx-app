...
def f_reports_time_wait_to_load(request: Request, pk=0) -> dict | str | None:
    if request.method == "GET":
        ...
        _args = dict(param_date=param_date, param_shift=param_shift, param_select_tech_id=param_select_tech_id, param_target=param_target)

        def __query() -> list | tuple | dict | str:
            trips: list[tuple] = django_utils.request_to_oracle(query=django_queries.query_time_wait_to_load(), args={**_args}, many=True)
