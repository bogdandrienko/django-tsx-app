...
export default function Page(): JSX.Element {
  ...
  useEffect(() => {
    dispatch(slices.analyse.tripsReadListStore.action({}));
    setTimeout(async () => {
  ...
  useEffect(() => {
    if (tripsReadListStore.data) {
      setMonitoring(tripsReadListStore.data);
    }
  ...
  return (
    <base.Base1
      title={"Анализ вывоза породы"}
      description={
        "на этой странице автоматически происходит обновление показателей КПД вывоза"
      }
    >
      ...
      <div>
        ...
        <div className={"input-group shadow p-3"}>
          ...
          <span>Выберите жёлтый уровень КПД (%) </span>
          <select
            onChange={(event) =>
              setterFormObj({
                ...getterFormObj,
          ...
          <span>Выберите красный уровень КПД (%) </span>
          <select
            onChange={(event) =>
              setterFormObj({
                ...getterFormObj,
                // @ts-ignore
                dangerLevel: parseInt(event.target.value, 10),
              })
            }
            className="form-select form-select-lg mb-3"
          >
