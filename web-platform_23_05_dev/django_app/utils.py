    ...
    def __request(func_controller: callable) -> any:
        ...
        def __wrapper(_request: Request, pk=0) -> Response:
            try:
                ...
                if auth is True and not _request.user.username:
                    raise Exception("Unauthorized")
                if _request.user.username:
                    update_last_login(sender=None, user=_request.user)

                result = func_controller(_request, pk)
                if result is None:
                    raise Exception("Method Not Allowed!")

                time_stop_func = time.perf_counter()  # TODO check func elapsed time

                if settings.LOGGING_ACTION_MODEL:
                    django_models.LoggingModel.objects.create(
                        user=_request.user if _request.user.username else None,
                        ip=_request.META.get("REMOTE_ADDR"),
                        path=_request.path,
                        method=_request.method,
                        text=f"[{round(time_stop_func - time_start_func, 5)}] " + str(result) if settings.LOGGING_RESPONSE_MODEL else "-",
                    )
                if settings.LOGGING_ACTION_FILE:
                    text = f"\ntime: {DateTimeUtils.get_current_time()} user: {_request.user if _request.user.username else None} ip: " \
                           f"{_request.META.get('REMOTE_ADDR')} path: {_request.path} method: {_request.method} action: " \
                           f"_request.action response: [{round(time_stop_func - time_start_func, 5)}] " \
                           f"{str(result) if settings.LOGGING_RESPONSE_FILE else '-'}"
                    with open('static/media/logging_actions.txt', 'a') as log:
                        log.write(text)
                if settings.LOGGING_ACTION_CONSOLE:
                    text = f"\ntime: {DateTimeUtils.get_current_time()} user: {_request.user if _request.user.username else None} ip: " \
                           f"{_request.META.get('REMOTE_ADDR')} path: {_request.path} method: {_request.method} action: " \
                           f"_request.action response: [{round(time_stop_func - time_start_func, 5)}] " \
                           f"{str(result) if settings.LOGGING_RESPONSE_CONSOLE else '-'}"
                    print(text)

                return Response({"response": result}, status.HTTP_200_OK)
            except Exception as error:
                if settings.LOGGING_ERROR_MODEL:
                    django_models.LoggingModel.objects.create(
                        user=_request.user if _request.user.username else None,
                        ip=_request.META.get('REMOTE_ADDR'),
                        path=_request.path + "_ERROR",
                        method=_request.method,
                        text=str(error)[:3000 - 1],
                    )
                if settings.LOGGING_ERROR_FILE:
                    text = f"\ntime: {DateTimeUtils.get_current_time()} user: {_request.user if _request.user.username else None} ip: " \
                           f"{_request.META.get('REMOTE_ADDR')} path: {_request.path}_ERROR method: {_request.method} action: " \
                           f"_request.action response: {str(error)[:3000 - 1]}"
                    with open('static/media/logging_actions.txt', 'a') as log:
                        log.write(text)
                if settings.LOGGING_ERROR_CONSOLE:
                    text = f"\ntime: {DateTimeUtils.get_current_time()} user: {_request.user if _request.user.username else None} ip: " \
                           f"{_request.META.get('REMOTE_ADDR')} path: {_request.path}_ERROR method: {_request.method} action: " \
                           f"_request.action response: {str(error)[:3000 - 1]}"
                    print(text)
                return Response({"error": str(error)}, status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class DjangoClass:
    ...
    class DRFClass:
        class RequestClass:
            def __init__(self, request: Request, pk: int):
                try:
...
def request_to_oracle(query: str, args: dict, many: bool) -> tuple | list[tuple]:
    # todo LINUX:
...
exit
    ...
    with cx_Oracle.connect('DISPATCHER/disp@172.30.23.194/PITENEW') as __connection:
