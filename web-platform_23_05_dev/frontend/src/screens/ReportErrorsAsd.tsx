...
export default function Page(): JSX.Element {
  ...
  async function GetMonitoringData() {
    try {
      const config = {
        url: `reports/errors_asd/?date=${getterFormObj.dateFrom}&shift=${getterFormObj.shiftFrom}&selectTechId=${getterFormObj.selectTechId}&target=${getterFormObj.target}`,
        method: `GET`,
        timeout: 30000,
        headers: {
          Authorization: `;`,
  ...
  return (
    <base.Base1
      title={"Отчёты: Анализ анализ ошибок по технике из АСД"}
      description={
        "на этой странице можно выгрузить отчёт по ошибкам в АСД за смену"
      }
    >
      ...
      <div>
        <div className={"input-group w-100 shadow p-3 w-25"}>
          ...
          >
            ...
            <option value={"Только 001 и 003"}>
              Только 001 и 003
            </option>
            <option value={"Cамосвал"}>
              Только самосвалы
            </option>
            <option value={"Экскаватор"}>
              Только экскаваторы
            </option>
            <option value={"Бульдозер"}>
              Только бульдозеры
            </option>
          </select>
          <button
