...
def f_events_drainage(request: Request, pk=0) -> dict | str | None:
    if request.method == "GET":
        ...
        def __query() -> list | dict | str:
            __rows: tuple = django_utils.request_to_oracle(query=django_queries.query_drainage_status(),
                                                           args={"timeDiff": time_diff}, many=False)
            __data = {
                "maxtime": __rows[0],
                "mintime": __rows[1],
                "maxfuel": __rows[2],
        ...
        data = django_utils.caching(LocMemCache, f"{request.path}_{request.method}_{time_diff}", timeout=9,
                                    query=__query)
...
def f_events_dumptrucks(request: Request, pk=0) -> dict | str | None:
    if request.method == "GET":
        ...
        def __query() -> list | dict | str:
            ...
            rows_instances = [
                {"vehid": i[0], "time": i[1], "x": i[2], "y": i[3], "weight": i[4], "fuel": i[5], "speed": i[6]} for i
                in rows_raw]
...
@django_utils.drf_decorator(auth=False)
def f_pto_operative_stoppages(request: Request, pk=0) -> dict | str | None:
    if request.method == "GET":
        if pk != 0:
            return None
        ...
        def __query() -> list | tuple | dict | str:
            trips: list[tuple] = django_utils.request_to_oracle(
                ...
                # )}, many=True,
                args={}, many=True,
                query=django_queries.query_operative_stoppages()
            )
            # print(trips)
        ...
        r_data = django_utils.caching(
            cache=LocMemCache, timeout=1, query=__query,
            name=f"{request.path}_{request.method}"
        )
...
@django_utils.drf_decorator(auth=False)
def f_pto_analytic_tech(request: Request, pk=0) -> dict | str | None:
    if request.method == "GET":
        if pk != 0:
            return None
        ...
        def __query() -> list | tuple | dict | str:
            trips: list[tuple] = django_utils.request_to_oracle(
                args={"p_minLength": 0.0}, many=True,
                query=django_queries.query_analyse_predictive()
            )
            ...
            def kpd_match_old(j):
                ...
                shov = matrix_shovels[
                    str(j[
                            2])]  # {'Вскрыша скальная': 2.5, 'Вскрыша рыхлая': 2.5, 'Вскрыша транзитная': 2.5, 'Руда скальная': 2.5, 'ВКП Скала': 2.5}
            ...
            volumes_by_hours: list[any] = django_utils.request_to_oracle(
                args={}, many=True,
                query=django_queries.query_get_volumes_by_hours()
            )
            volumes_by_hours = [{"name": x[0], "объём": float(x[1]), "pv": 2400, "amt": 2400} for x in volumes_by_hours]

            volumes_by_category: list[any] = django_utils.request_to_oracle(
                args={}, many=True,
                query=django_queries.query_get_volumes_by_category()
            )
            volumes_by_category = [
                {"name": x[0] + f" | {x[2]} рейса(-ов)", "объём": float(x[1]), "pv": 2400, "amt": 2400}
                for x in volumes_by_category
            ]
            elapsed_time_raw: tuple[any] = django_utils.request_to_oracle(
                args={}, many=False,
                query=django_queries.query_get_elapsed_time()
            )

            extra = {"volumes_by_hours": volumes_by_hours, "volumes_by_category": volumes_by_category,
                     "trips_count": len(trips_raw), "elapsed_time": elapsed_time_raw[2]}
            return data4, trips_raw, extra

        r_data, r_trips, r_extra = django_utils.caching(
            cache=LocMemCache, timeout=5, query=__query,
            name=f"{request.path}_{request.method}"
        )

        return {"data": r_data, "trips": r_trips, "extra": r_extra}
...
def f_analyse_predictive(request: Request, pk=0) -> dict | str | None:
    if request.method == "GET":
        ...
        high_correct = int(request.GET.get("highCorrect", 0))
        length_correct = int(request.GET.get("lengthCorrect", 0))
        weather_correct = int(request.GET.get("whetherCorrect", 100))
        min_length = float(request.GET.get("minLength", 1.0))

        def __query() -> list | tuple | dict | str:
            trips: list[tuple] = django_utils.request_to_oracle(query=django_queries.query_analyse_predictive(),
                                                                args={"p_minLength": min_length}, many=True)
            ...
            def kpd_match(j):
                ...
                full = round(((path + high + length) * (100 / veh["Состояние, %"]) * (100 / weather_correct)), 1)
                # print(weather_correct, full)
                kpd = round((full / elapsed_time1) * 100, 0)

                return kpd, f"({path}[путь] + {length}[корр. манёвров] + {high}[корр. высоты]) * " \
                            f"{veh['Состояние, %']}[состояние самосвала] * " \
                            f"{veh['Погода, %']}[погода] = {full}[итого норма] | {round(elapsed_time1, 1)}[факт]", 0
            ...
            def kpd_match_old(j):
                ...
                return kpd, f"({load}[погрузка] + {path}[путь] + {unload}[разгрузка] + {return_path}[возврат]) * " \
                            ...
                            f"{round(elapsed_time1, 1)}[факт]", return_path
        ...
        r_data, r_trips = django_utils.caching(LocMemCache, f"{request.path}_{request.method}", timeout=2,
                                               query=__query)
    ...
    if request.method == "GET":
        ...
        def __query() -> list | tuple | dict | str:
            trips: list[tuple] = django_utils.request_to_oracle(
                ...
                args=dict(param_date=param_date, param_shift=param_shift, param_select_tech_id=param_select_tech_id,
...
def f_reports_avg_speed(request: Request, pk=0) -> dict | str | None:
    if request.method == "GET":
        ...
        def __query_speed_by_hours() -> list | tuple | dict | str:
            trips: list[tuple] = django_utils.request_to_oracle(
                query=django_queries.query_analyse_avg_speed_by_hours(),
        ...
        def __query(shift) -> list | tuple | dict | str:
            trips: list[tuple] = django_utils.request_to_oracle(
                query=django_queries.query_analyse_avg_speed(),
                args=dict(param_date=param_date, param_shift=shift, param_select_tech_id=param_select_tech_id),
                many=True
            )
            ...
            def kpd(_trips: list[dict]) -> dict:
                ...
                if len(_trips) < 2:
                    return {"avgloadspeed": _trips[0]["avgloadspeed"],
                ...
                return {"avgloadspeed": _avgloadspeed, "avgemptyspeed": _avgemptyspeed, "avgspeed": _avgspeed,
                        "last_loadspeed": _trips[-1]["avgloadspeed"], "last_emptyspeed": _trips[-1]["avgemptyspeed"],
                        "last_speed":
        ...
        r_trips = django_utils.caching(
            ...
            timeout=1, query=__query_get_day_and_night
        )
        ...
        return {"data": r_trips, "lines": lines, "query": {"date": param_date, "shift": param_shift}}
        ...
        def __query() -> list | dict | str:
            ...
            for i in rows_raw:
                ...
                if summ_mass > 0:
                    ...
                    km = 0
                temp_row_instances = [
                    [i[3], "тр,км", "тр", round(i[4], round_point), round(i[5], round_point), 2.1, tr_mass,
