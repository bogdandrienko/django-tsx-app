...
export default function Page(): JSX.Element {
  ...
  async function GetMonitoringData() {
    try {
      const config = {
        url: `${constants.SERVER_HOST_AND_PORT_CONSTANT}reports/time_wait_to_load/?date=${getterFormObj.dateFrom}&shift=${getterFormObj.shiftFrom}&selectTechId=${getterFormObj.selectTechId}&target=${getterFormObj.target}`,
        method: `GET`,
        timeout: 30000,
        headers: {
          Authorization: `;`,
              ...
              <table className="table table-light table-hover table-striped m-0 p-0">
                <thead>
                  <tr>
                    ...
                    <th scope="col">
                      Среднее время ожидания под погрузку (только рейсы с
                      ожиданием выше 2 минут)
                    </th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td
                      className={
                        monitoring.idle.sum_idles > 1500 ? "text-danger" : ""
                      }
                    >
                      {monitoring.idle.sum_idles}
                    </td>
                    <td
                      className={
                        monitoring.idle.avg_wait > 4.5 ? "text-danger" : ""
                      }
                    >
                      {monitoring.idle.avg_wait}
                    </td>
                  </tr>
                </tbody>
              </table>
