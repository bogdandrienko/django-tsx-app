...
export default function Page(): JSX.Element {
  ...
  const ideaCreateStore = hooks.useSelectorCustom1(slices.old.taskPostStore);
    ...
    if (ideaCreateStore.data) {
      utils.Delay(() => {
        if (ideaCreateStore.data) {
          ...
          dispatch({ type: slices.old.taskPostStore.constant.reset });
          navigate("/tasks");
        }
      }, 100);
    }
  ...
  function CreateIdea() {
    dispatch(
      slices.old.taskPostStore.action({ form: { ...ideaCreateObject } })
    );
  }
  ...
  return (
    ...
    <bases.Base1 title={"Create task page"} description={"create new task"}>
      ...
      <components.StatusStore1
        slice={slices.old.taskPostStore}
