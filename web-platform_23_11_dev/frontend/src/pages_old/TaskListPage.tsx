...
export default function Page(): JSX.Element {
  ...
  const taskReadListStore = hooks.useSelectorCustom1(
    slices.old.taskReadListStore
  );
  const userReadListStore = hooks.useSelectorCustom1(
    slices.old.userReadListStore
  );
  const taskDeleteStore = hooks.useSelectorCustom1(slices.old.taskDeleteStore);
  ...
  useEffect(() => {
    if (!userReadListStore.data) {
      dispatch(slices.old.userReadListStore.action({}));
    }
  ...
  useEffect(() => {
    if (taskDeleteStore.data) {
      utils.Delay(() => {
        dispatch({ type: slices.old.taskDeleteStore.constant.reset });
        dispatch({ type: slices.old.taskReadListStore.constant.reset });
      }, 300);
    }
  ...
  useEffect(() => {
    if (!taskReadListStore.data) {
      dispatch(
        slices.old.taskReadListStore.action({
  ...
  useEffect(() => {
    ...
    dispatch({ type: slices.old.userReadListStore.constant.reset });
    dispatch({ type: slices.old.taskReadListStore.constant.reset });
  }, []);
  ...
  function DeleteTask() {
    dispatch(
      slices.old.taskDeleteStore.action({
                ...
                <div className="card-header m-0 p-0">
                  <div className="m-0 p-0">
                    ...
                    <components.StatusStore1
                      slice={slices.old.userReadListStore}
                      consoleLog={constants.DEBUG_CONSTANT}
                      showLoad={false}
                      showData={false}
                    />
                    <components.StatusStore1
                      slice={slices.old.taskReadListStore}
                      consoleLog={constants.DEBUG_CONSTANT}
                      showData={false}
                    />
                  </div>
