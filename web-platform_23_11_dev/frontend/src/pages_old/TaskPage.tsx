...
export default function Page(): JSX.Element {
  ...
  const taskReadStore = hooks.useSelectorCustom1(slices.old.taskReadStore);
  ...
  useEffect(() => {
    if (!taskReadStore.data) {
      dispatch(
        slices.old.taskReadStore.action({
  ...
  useEffect(() => {
    if (taskDeleteStore.data) {
      utils.Delay(() => {
        ...
        dispatch({ type: slices.old.taskReadStore.constant.reset });
        navigate("/tasks");
      }, 300);
    }
  ...
  return (
    ...
    >
      ...
      <components.StatusStore1
        slice={slices.old.taskReadStore}
