...
class Speed:
    ...
    @staticmethod
    @router.get("/api/speed/report/dumptrucks_custom", response_class=JSONResponse)
    @utils.decorator(need_auth=False)
    async def get_target_report_avg_speed_custom(request: Request):
        param_date: datetime.datetime = datetime.datetime.strptime(request.query_params["param_date"], "%Y-%m-%d")
        ...
        def __query() -> dict:
            trips_raw: list[tuple] = utils.request_to_oracle(
                query=queries.Develop.get_target_report_avg_speed_custom(),
                args={"param_date": param_date, "param_shift": param_shift},
                many=True,
                is_fake=False,
                fake_file_path="static_external/fake/get_target_report_avg_speed_custom.xlsx",
            )
...
class Pto:
    ...
    @staticmethod
    @router.get("/api/pto/monitoring/sticking", response_class=JSONResponse)
    @utils.decorator(need_auth=False)
    async def pto_monitoring_sticking(request: Request):
        param_date: datetime.datetime = datetime.datetime.strptime(request.query_params["param_date"], "%Y-%m-%d")
        param_shift: int = int(request.query_params["param_shift"])
        param_min_speed: int = int(request.query_params["param_min_speed"])
        param_min_weight: int = int(request.query_params["param_min_weight"])
        ...
        def __query() -> dict:
            trips_raw: list[tuple] = utils.request_to_oracle(
                query=queries.Pto.get_pto_monitoring_sticking(),
                args={},
                many=True,
                is_fake=False,
                fake_file_path="static_external/fake/get_pto_monitoring_sticking.xlsx",
            )
            # print("trips_raw: ", trips_raw[0:3])
            """
[
[0,             1,      2,          3,          4,      5,          6,          7], 
('TASKDATE', 'SHIFT', 'TIMELOAD', 'VEHID', 'SHOVID', 'WORKTYPE', 'WEIGHT', 'AVWEIGHT_EMPTY'), 
(datetime.datetime(2023, 12, 13, 0, 0), 2, datetime.datetime(2023, 12, 13, 8, 0, 45), '107', '001', 'Вскрыша рыхлая', 98, 2)
]
"""
            trips_dict = [
                {
                    ...
                    "WORKTYPE": str(i[5]),
                    "WEIGHT": int(i[6]),
                    "WEIGHT_EMPTY": int(i[7]),
                    "TIMELOAD": i[2],
                }
                for i in trips_raw[2:]
            ]
            # группировка
            _trips_dict_by_veh_id: dict[str, list[dict]] = {}
            for i in trips_dict:
                try:
                    _trips_dict_by_veh_id[i["VEHID"]].append(i)
                except Exception as _:
                    _trips_dict_by_veh_id[i["VEHID"]] = [i]

            __data: list[dict] = []
            # запуск цикла по каждому самосвалу
            for k, trips_by_veh_id in _trips_dict_by_veh_id.items():
                count_sticking = 0
                weight_sticking = 0
                timeload = ""
                for trip in trips_by_veh_id[-3:]:
                    if trip["WEIGHT_EMPTY"] > 5:
                        count_sticking += 1
                    weight_sticking = trip["WEIGHT_EMPTY"]
                    timeload = trip["TIMELOAD"]
                __data.append({"vehid": k, "count_sticking": count_sticking, "weight_sticking": weight_sticking, "timeload": timeload})

            __data = sorted(__data, key=lambda x: x["vehid"], reverse=False)

            return {"data": __data, "parameters": __parameters}
