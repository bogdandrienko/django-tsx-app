...
export default function Page(): JSX.Element {
  ...
  const taskUpdateStore = hooks.useSelectorCustom1(slices.old.taskUpdateStore);
  ...
  useEffect(() => {
    if (!taskReadStore.data) {
      dispatch(
        slices.old.taskReadStore.action({ form: { addToUrl: `/${id}` } })
      );
    }
  }, [taskReadStore.data]);
  ...
  function resetIdea() {
    ...
    dispatch({ type: slices.old.taskUpdateStore.constant.reset });
  }

  function UpdateIdea() {
    dispatch(
      slices.old.taskUpdateStore.action({
  ...
  return (
    ...
    >
      ...
      />
      <components.StatusStore1
        slice={slices.old.taskUpdateStore}
        consoleLog={constants.DEBUG_CONSTANT}
        showLoad={true}
        loadText={""}
        showData={false}
